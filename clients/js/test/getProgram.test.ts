...
import {
  ...
  TOOLS_ID as MPL_AGENT_TOOLS_PROGRAM_ID,
} from '../src';
...
test('it registers the tools program', async (t) => {
  // Given a Umi instance using the project's plugin.
  const umi = await createUmi();

  // When we fetch the registered program.
  const program = umi.programs.get('mplAgentTools');

  // Then we expect it to be the same as the program ID constant.
  t.true(program.publicKey === MPL_AGENT_TOOLS_PROGRAM_ID);
});
