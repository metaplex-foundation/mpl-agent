...
pub fn register_validation_v1<'a>(
    ...
    // Agent validation PDA must not already be initialized. System CreateAccount
    ...
    if ctx.accounts.agent_validation.data_len() != 0
        || *ctx.accounts.agent_validation.owner != system_program::id()
    {
        return Err(MplAgentValidationError::AgentValidationAlreadyRegistered.into());
    }
    ...
    // only thing preventing unauthorized validation registration in that
    // branch.
    ...
    let result = fetch_wrapped_external_plugin_adapter::<BaseAssetV1>(
        ...
        &ExternalPluginAdapterKey::AppData(PluginAuthority::Address {
            address: *ctx.accounts.agent_validation.key,
        }),
    );
