...
const _: () = assert!(core::mem::size_of::<AgentValidationV1>() % 8 == 0);
const _: () = assert!(core::mem::size_of::<AgentValidationV1>() == 40);

impl AgentValidationV1 {
    /// PDA seed prefix for this account type.
    pub const PREFIX: &'static [u8] = b"agent_validation";
    ...
    pub fn create_account(accounts: &RegisterValidationV1Accounts, bump: u8) -> ProgramResult {
        solana_program::msg!("Creating agent validation account");
        create_or_allocate_account_raw(
            crate::ID,
            accounts.agent_validation,
            accounts.system_program,
            accounts.payer,
            core::mem::size_of::<AgentValidationV1>(),
    ...
    pub fn initialize(&mut self, bump: u8, asset: &Pubkey) {
        solana_program::msg!("Initializing agent validation account");
        self.key = Key::AgentValidationV1 as u8;
        self.bump = bump;
        self._padding = [0u8; 6];
        self.asset = *asset;
    }
