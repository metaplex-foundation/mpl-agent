...
use crate::{error::Mpl8004IdentityError, instruction::accounts::RegisterV1Accounts};
...
const _: () = assert!(core::mem::size_of::<CollectionConfigV1>() % 8 == 0);
const _: () = assert!(core::mem::size_of::<CollectionConfigV1>() == 40);

impl CollectionConfigV1 {
    /// PDA seed prefix for this account type.
    pub const PREFIX: &'static [u8] = b"collection_config";
    ...
    pub fn check_pda_derivation(
        ...
        assert_derivation(
            ...
            Mpl8004IdentityError::InvalidAccountData,
        )
    }

    pub fn create_account(accounts: &RegisterV1Accounts, bump: u8) -> ProgramResult {
        solana_program::msg!("Creating collection config account");
        create_or_allocate_account_raw(
            crate::ID,
            accounts.collection_config,
            accounts.system_program,
            accounts.payer,
            core::mem::size_of::<CollectionConfigV1>(),
    ...
    pub fn initialize(&mut self, bump: u8, collection: &Pubkey) {
        solana_program::msg!("Initializing collection config account");
        self.key = Key::CollectionConfigV1 as u8;
        self.bump = bump;
        self._padding = [0u8; 6];
