...
use crate::processor::{
    DepositSubsidyV1Args, InitializeReviewsConfigV1Args, LeaveReviewV1Args,
    RegisterReputationV1Args, RegisterReviewsTreeV1Args, WithdrawSubsidyV1Args,
};
...
pub enum MplAgentReputationInstructionDiscriminant {
    ...
    DepositSubsidyV1 = 2,
    WithdrawSubsidyV1 = 3,
    InitializeReviewsConfigV1 = 4,
    RegisterReviewsTreeV1 = 5,
}
...
impl TryFrom<u8> for MplAgentReputationInstructionDiscriminant {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            2 => Ok(MplAgentReputationInstructionDiscriminant::DepositSubsidyV1),
            3 => Ok(MplAgentReputationInstructionDiscriminant::WithdrawSubsidyV1),
            4 => Ok(MplAgentReputationInstructionDiscriminant::InitializeReviewsConfigV1),
            5 => Ok(MplAgentReputationInstructionDiscriminant::RegisterReviewsTreeV1),
            _ => Err(()),
        }
    }
}
...
pub enum MplAgentReputationInstruction {
    ...
    #[account(4, name="program_config", desc = "Singleton ProgramConfigV1 PDA. Signs the Bubblegum CPI as tree creator/delegate via invoke_signed")]
    ...
    #[account(16, writable, optional, name="subsidy_pool", desc = "Optional ReviewSubsidyPoolV1 PDA for this agent; if present, the review record's rent is refunded to payer from the pool")]
    #[account(17, name="system_program", desc = "The system program")]
    LeaveReviewV1(LeaveReviewV1Args),

    /// Initialize and/or top up the subsidy pool for an agent. Funding it
    /// is permissionless (anyone can deposit) but only the
    /// `withdraw_authority` captured at first initialization can later
    /// pull funds out via `WithdrawSubsidyV1`.
    #[account(0, writable, signer, name="payer", desc = "Funds the deposit (and pool rent on first init)")]
    #[account(1, name="agent_asset", desc = "The Core asset representing the agent")]
    #[account(2, writable, name="subsidy_pool", desc = "ReviewSubsidyPoolV1 PDA")]
    #[account(3, optional, name="withdraw_authority", desc = "Captured as the pool's withdraw authority on first init (defaults to payer)")]
    #[account(4, name="system_program", desc = "The system program")]
    DepositSubsidyV1(DepositSubsidyV1Args),

    /// Withdraw lamports from a subsidy pool. Only the pool's
    /// `withdraw_authority` (set at first deposit) may call this. The
    /// pool's own rent-exempt minimum is preserved.
    #[account(0, writable, signer, name="withdraw_authority", desc = "Must match the pool's withdraw_authority")]
    #[account(1, name="agent_asset", desc = "The Core asset representing the agent")]
    #[account(2, writable, name="subsidy_pool", desc = "ReviewSubsidyPoolV1 PDA")]
    #[account(3, writable, name="destination", desc = "Where to send the withdrawn lamports")]
    WithdrawSubsidyV1(WithdrawSubsidyV1Args),

    /// Bootstrap: create the singleton ProgramConfigV1 PDA and the
    ...
    /// initialize_program_config_v1) for cross-program canonicalisation.
