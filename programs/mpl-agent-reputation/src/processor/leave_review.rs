...
use crate::{
    ...
    state::{
        ...
        MPL_ACCOUNT_COMPRESSION_ID, REVIEWS_AUTHORITY_PREFIX,
    },
};
...
/// Fixed-head Pod args for the LeaveReviewV1 instruction. The
/// `feedback_uri` is a zero-sized sentinel; the actual UTF-8 bytes are
/// length-prefixed (u32 LE) and appended after this struct in the
/// instruction data. Kinobi renders `feedbackUri: string` in the
/// generated clients via the `#[idl_type("String")]` annotation.
///
/// The review references a work receipt by its `(merkle_tree, nonce)`
/// pair — the Bubblegum asset id is deterministically derived from
/// those. Proof data accompanies the call so we can validate the
/// receipt on-chain.
#[repr(C)]
...
pub struct LeaveReviewV1Args {
    ...
    /// Star rating, 1..=5.
    pub rating: u8,
    /// Padding to align the `u64` fields that follow.
    #[padding]
    pub _pad: [u8; 6],
    ...
    /// Padding to keep subsequent fields on their natural alignment.
    #[padding]
    pub _pad2: [u8; 3],
    ...
    /// URI of the off-chain review JSON file. Zero-sized sentinel; the
    /// real UTF-8 bytes are length-prefixed and appended after this
    /// struct in the instruction data.
    #[idl_type("String")]
    pub feedback_uri: [u8; 0],
}
const _: () = assert!(core::mem::size_of::<LeaveReviewV1Args>() == 136);
const _: () = assert!(core::mem::align_of::<LeaveReviewV1Args>() == 8);
...
pub fn leave_review_v1<'a>(
    ...
    // Split raw instruction data into the Pod head and the
    // length-prefixed feedback URI tail.
    if instruction_data.len() < core::mem::size_of::<LeaveReviewV1Args>() {
        return Err(MplAgentReputationError::InvalidInstructionData.into());
    }
    let (args_bytes, tail) = instruction_data.split_at(core::mem::size_of::<LeaveReviewV1Args>());
    let args: &LeaveReviewV1Args = from_bytes(args_bytes);
    let feedback_uri = read_length_prefixed_string(tail, MAX_FEEDBACK_URI_LEN)?;
    ...
    // Bind the receipt to the reviewed agent AND the reviewing client.
    // MintWorkReceiptV1 always writes:
    //   creators = [
    //       {address: agent_asset, verified: false, share: 100},
    //       {address: client,      verified: false, share: 0  },
    //   ]
    // so we can compute the expected creator_hash on-chain from
    // `ctx.accounts.asset.key` and `ctx.accounts.reviewer.key`. This:
    //   1. Prevents replay of a real receipt for AgentA against a review
    //      for AgentB — the reconstructed leaf hash would no longer match
    //      the receipt's actual leaf in the tree.
    //   2. Cryptographically gates LeaveReviewV1 on the reviewer being
    //      the client the receipt was minted for, with no signer
    //      handshake at mint time. A stranger who somehow owned the leaf
    //      could not reproduce this hash with their own pubkey — the
    //      verify_leaf CPI would reject.
    let expected_creator_hash = keccak::hashv(&[
        ctx.accounts.asset.key.as_ref(),
        &[0u8],
        &[100u8],
        ctx.accounts.reviewer.key.as_ref(),
        &[0u8],
        &[0u8],
    ])
    .to_bytes();
