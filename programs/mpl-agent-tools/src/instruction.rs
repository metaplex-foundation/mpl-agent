...
pub enum MplAgentToolsInstruction {
    ...
    /// co-signs the mint as a handshake. The receipts authority PDA
    /// (constant program signer) authorizes both the tree write and the
    /// collection add — caller does not need to provide an admin signer.
