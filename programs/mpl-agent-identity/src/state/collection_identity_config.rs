...
pub struct CollectionIdentityConfigV1 {
...
const _: () = assert!(core::mem::size_of::<CollectionIdentityConfigV1>() % 8 == 0);
const _: () = assert!(core::mem::size_of::<CollectionIdentityConfigV1>() == 40);

impl CollectionIdentityConfigV1 {
    /// PDA seed prefix for this account type.
    pub const PREFIX: &'static [u8] = b"collection_identity_config";
    ...
    pub fn create_account(accounts: &RegisterIdentityV1Accounts, bump: u8) -> ProgramResult {
        solana_program::msg!("Creating collection identity config account");
        create_or_allocate_account_raw(
            crate::ID,
            accounts.collection_identity_config,
            accounts.system_program,
            accounts.payer,
            core::mem::size_of::<CollectionIdentityConfigV1>(),
    ...
    pub fn initialize(&mut self, bump: u8, collection: &Pubkey) {
        solana_program::msg!("Initializing collection identity config account");
        self.key = Key::CollectionIdentityConfigV1 as u8;
        self.bump = bump;
        self._padding = [0u8; 6];
